...
(:~
 ...
 :)
module namespace file = "http://www.28msec.com/cloud9/lib/file";

import module namespace fs = "http://www.28msec.com/cloud9/lib/fs";
...
{
  if(file:is-directory($file)) then
    error(xs:QName('file:FOFL0004'))
  else
    fs:append-binary($file, $content)
};
...
) as empty-sequence()
{
  if (file:exists($source)) then
    ...
    fn:error(xs:QName("file:FOFL0001"), "The source path does not exists: " || $source)
};
...
) as empty-sequence()
{
  if(not(file:exists($sourceFile))) then {
    error(xs:QName('file:FOFL0001'))
  } else if(file:is-directory($sourceFile)) then {
    error(xs:QName('file:FOFL0004'))
  } else if(
    not(file:exists($destination)) and
    not(file:is-directory(file:dir-name($destination)))
  ) then {
    error(xs:QName('file:FOFL0003'))
  } else {
    variable $dest := if(file:is-directory($destination)) then $destination || file:directory-separator() || file:base-name($sourceFile) else $destination;
    file:write-binary($dest, file:read-binary($sourceFile));
  }
};
...
) as empty-sequence()
{
  if(file:exists($dir)) then
    error(xs:QName('file:FOFL0002'))
  else
    let $dir := file:path-to-native($dir)
    let $segments := tokenize($dir, file:directory-separator())
    for $segment at $i in $segments
    let $full-path := string-join(subsequence($segments, 1, $i), file:directory-separator())
    return
      if(not(file:exists($full-path))) then
        fs:create-directory($full-path)
      else if(not(file:is-directory($full-path))) then
        error(xs:QName('file:FOFL0002'))
      else ()
};
...
) as xs:base64Binary
{
  if(not(file:exists($file))) then
    error(xs:QName('file:FOFL0001'))
  else if(file:is-directory($file)) then
    error(xs:QName('file:FOFL0004'))
  else
    fs:read-binary($file)
};
...
(:~
 ...
 :)
declare function file:directory-separator() as xs:string
{
  $fs:directory-separator
};
...
(:~
 ...
 :)
declare function file:path-separator() as xs:string
{
  $fs:path-separator
};
...
(:~
 ...
 :)
declare function file:path-to-native($path as xs:string) as xs:string
{
  fs:path-to-native($path)
};
